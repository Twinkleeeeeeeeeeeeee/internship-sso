package com.internship.mvc.repository;
...
public interface UserRepository extends PagingAndSortingRepository<User, Long>, JpaSpecificationExecutor<User> {
    ...
    long getActiveUserCount(@Param(value = "platform") String platform, @Param(value = "updatedDate") String updatedDate);

    //通过手机号或者邮箱模糊查询
    @Query(value = "select u from User u where u.email like %?1% or u.iphone like %?2% ")
    List<User> findByEmailorfindByPhone(String email, String phone);

    //修改某个用户邮箱
    @Modifying
    @Query(value = "update User u set u.email=:email where u.id=:id")
    void updateUserEmail(@Param(value = "email") String email, @Param(value = "id") String id);
}
